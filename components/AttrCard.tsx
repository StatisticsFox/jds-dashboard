            ...
            <li key={r.label} className="group">
              <div className="flex items-baseline justify-between gap-2">
                ...
                <span className="shrink-0 text-xs">
                  <span className="text-muted">{r.count}명</span>{" "}
                  <b className="ml-1">{pct.toFixed(0)}%</b>
                </span>
              </div>
...
export function MbtiCard({
  ...
  axes: { a: string; b: string; na: number; nb: number }[];
  top: Row[];
  total: number;
}) {
  return (
    <section className="card flex flex-col p-5">
      <h3 className="text-base">MBTI</h3>
      <ul className="mt-3 space-y-2.5 text-xs tabular-nums">
        {axes.map(({ a, b, na, nb }) => {
          ...
          return (
            <li
              key={a}
              className="grid grid-cols-[3.25rem_1fr_3.25rem] items-center gap-2"
            >
              <span>
      ...
      <div className="mt-4 border-t border-grid pt-3">
        ...
        <ol className="flex flex-wrap gap-1.5 text-xs">
          {top.map((t) => (
            <li
              key={t.label}
              className="rounded-full bg-accent-soft px-2.5 py-1"
            >
              <b className="font-mono">{t.label}</b>{" "}
              <span className="text-muted">
                {t.count}명 · {total ? Math.round((t.count / total) * 100) : 0}%
              </span>
            </li>
          ))}
        </ol>
...
export function CompareTable({
  ...
  const pct = (r: (typeof rows)[number], v: string) =>
    r.total ? (r.counts.get(v) ?? 0) / r.total : 0;
  const max = Math.max(
    0.0001,
    ...rows.flatMap((r) => values.map((v) => pct(r, v))),
  );
  return (
    <CrossTable>
            ...
            <tr className="text-muted">
              ...
              {values.map((v, j) => (
                <th
                  ...
                  data-r="h"
                  data-c={j}
                  className="min-w-12 max-w-24 break-keep px-1.5 py-2 text-center align-bottom font-medium leading-tight"
                >
                  {v}
                </th>
              ))}
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr
                key={r.key}
                className={
                  r.active ? "outline-2 -outline-offset-1 outline-accent" : ""
                }
              >
                <th
                  data-r={i}
                  data-c="h"
                  className={`sticky left-0 z-10 whitespace-nowrap bg-surface px-2 py-1.5 text-left ${r.key.startsWith("y") || r.key === "all" ? "border-t border-border" : ""}`}
                >
                  <PendingLink
                    href={r.href}
                    className={`rounded-md px-1 hover:underline ${r.active ? "font-semibold text-accent-strong" : "font-medium"}`}
                  >
                    {r.label}
                  </PendingLink>
                </th>
                ...
                {values.map((v, j) => {
                  ...
                  return (
                    <td
                      ...
                      data-r={i}
                      data-c={j}
                      data-tip={`${r.label} · ${v}\n${n}명 · 등록 인원 중 ${(s * 100).toFixed(1)}%`}
                      className="rounded-md px-1.5 py-1.5 text-center"
                      style={{
                        background: n
                          ? `color-mix(in oklab, var(--series-1) ${Math.round(8 + (s / max) * 62)}%, var(--surface))`
                          : undefined,
                      }}
                    >
                      {n ? (
