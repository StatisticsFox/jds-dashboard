...
// 범례(또는 선 끝 이름표)에 올리면 그 선만 진하게, 범례를 누르면 그 선을 숨기거나 다시 보임
export function LineChart({
  ...
  // 강조 중인 선이 아니면 흐리게
  const dim = (id: string) => (focus !== null && focus !== id ? 0.15 : 1);
  ...
  const x = (i: number) =>
    PAD.left +
    (xLabels.length <= 1 ? innerW / 2 : (innerW * i) / (xLabels.length - 1));
  ...
  const path = (values: (number | null)[]) =>
    values.reduce(
      (d, v, i) =>
        v === null
          ? d
          : `${d}${i === 0 || values[i - 1] === null ? "M" : "L"}${x(i)},${y(v)}`,
      "",
    );
  ...
  function onMove(e: React.PointerEvent<SVGSVGElement>) {
    ...
    setHover(
      Math.min(
        xLabels.length - 1,
        Math.max(0, Math.round((px - PAD.left) / step)),
      ),
    );
  }
  ...
  const tip =
    hover === null
      ? null
      : shown
          .map((s) => ({ s, v: s.values[hover], d: s.deltas?.[hover] }))
          ...
          {series.map((s) => {
            ...
            return (
              <li key={s.id}>
                <button
                  type="button"
                  onPointerEnter={() => !off && setFocus(s.id)}
                  onClick={() => {
                    toggle(s.id);
                    setFocus(null);
                  }}
                  ...
                  className={`flex items-center gap-1.5 rounded-md px-2 py-1 transition-colors hover:bg-background ${off ? "text-muted/50 line-through" : ""}`}
                >
                  <span
      ...
      >
        ...
        {xLabels.map((l, i) => (
          <text
            ...
            y={height - 8}
            textAnchor="middle"
            className={`text-[11px] ${hover === i ? "fill-foreground font-semibold" : "fill-muted"}`}
          >
            {l}
          </text>
        ))}
        ...
        {[...shown]
          ...
          .map((s) => (
            <g key={s.id} opacity={dim(s.id)} className="transition-opacity">
              <path
                d={path(s.values)}
                fill="none"
                stroke={s.color}
      ...
      {tip && hover !== null && (
        ...
        >
          ...
          {tip.map(({ s, v, d }) => (
            <div
              key={s.id}
              className={`flex items-center justify-between gap-4 py-0.5 ${focus !== null && focus !== s.id ? "opacity-40" : ""}`}
            >
...
function niceStep(raw: number) {
  ...
  return Math.max(
    1,
    [1, 2, 5, 10].map((m) => m * pow).find((m) => m >= raw) ?? raw,
  );
}
