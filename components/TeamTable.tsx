...
function tint(diff: number, base: number) {
  ...
  return {
    backgroundColor: `color-mix(in oklab, ${color} ${level}%, transparent)`,
  };
}
        ...
        <table className="w-full min-w-[900px] border-separate border-spacing-0.5 text-sm tabular-nums">
          <thead>
            <tr className="text-xs text-muted">
              <th />
              <th
                colSpan={COUNT_METRICS.length}
                className="pb-1 text-left font-medium"
              >
                수치
              </th>
              <th
                colSpan={RATE_METRICS.length}
                className="pb-1 text-left font-medium"
              >
                유월율
              </th>
            ...
            <tr className="text-xs">
              ...
              {[...COUNT_METRICS, ...RATE_METRICS].map((m) => (
                <th
                  ...
                  data-r="h"
                  data-c={m.key}
                  className="whitespace-nowrap px-2 pb-2 text-right font-medium"
            ...
            {teams.map((t) => (
              <tr key={t.id}>
                <th
                  data-r={t.id}
                  data-c="h"
                  className="whitespace-nowrap pr-2 text-left font-medium"
                >
                  {t.emoji} {t.label}
                </th>
                {COUNT_METRICS.map((m) => {
                  ...
                  return (
                    <td
                      ...
                      data-r={t.id}
                      data-c={m.key}
                      data-tip={`${t.emoji} ${t.label} · ${m.label}\n${formatCount(t.counts[m.key])}명 · 팀 평균 ${formatCount(Math.round(average[m.key] * 10) / 10)}명 대비 ${formatCountDiff(diff)}`}
                      className="rounded-lg px-2 py-1.5 text-right"
                      style={tint(diff, average[m.key])}
                    >
                      <div className="font-semibold">
                        {formatCount(t.counts[m.key])}
                      </div>
                ...
                {RATE_METRICS.map((m) => {
                  ...
                  const diff =
                    value !== null && base !== null ? value - base : null;
                  return (
                    <td
                      key={m.key}
                      data-r={t.id}
                      data-c={m.key}
                      data-tip={`${t.emoji} ${t.label} · ${m.label}\n${formatRate(value)} · 지역 ${formatRate(base)}${diff !== null ? ` (${formatRateDiff(diff)})` : ""}`}
                      className="rounded-lg px-2 py-1.5 text-right"
                      style={diff !== null ? tint(diff, base!) : undefined}
                    >
                      <div className="font-semibold">{formatRate(value)}</div>
                      <div className="text-xs text-muted">
                        {diff !== null ? formatRateDiff(diff) : " "}
                      </div>
                    </td>
                  );
                })}
          ...
          <tfoot className="text-muted">
            <tr>
              <th className="whitespace-nowrap border-t border-border pr-2 pt-2 text-left font-medium">
                팀 평균
              </th>
              {COUNT_METRICS.map((m) => (
                <td
                  key={m.key}
                  className="border-t border-border px-2 pt-2 text-right"
                >
            ...
            <tr>
              ...
              {RATE_METRICS.map((m) => (
                <td
                  key={m.key}
                  className="px-2 text-right font-semibold text-foreground"
                >
