...
export function ReasonHeatmap({
  ...
  const maxShare = Math.max(
    0.0001,
    ...rows.flatMap((row) => reasons.map((r) => share(row.data, r))),
  );
    ...
    <CrossTable>
      <div className="-mx-5 overflow-x-auto px-5">
        <table className="w-full min-w-max border-separate border-spacing-0.5 text-xs tabular-nums">
          ...
          <tbody>
            {rows.map((row, i) => {
              const dropped = [...row.data.reasons.values()].reduce(
                (a, b) => a + b,
                0,
              );
              return (
                ...
                >
                  <th
                    data-r={i}
                    data-c="h"
                    className={`sticky left-0 z-10 bg-surface px-2 py-1.5 text-left ${row.key === "all" ? "border-t border-border" : ""}`}
                  >
                    <PendingLink
                      href={row.href}
                      className={`rounded-md px-1 hover:underline ${row.active ? "font-semibold text-accent-strong" : "font-medium"}`}
                    >
                      {row.label}
                    </PendingLink>
                  </th>
                  <td
                    data-r={i}
                    data-c="rate"
                    data-tip={`${row.label} ${rateLabel}\n${unit} ${row.data.finds}명 중 탈락 ${dropped}명`}
                    className="px-2 py-1.5 text-right text-muted"
                  >
                    {row.data.finds
                      ? `${Math.round((dropped / row.data.finds) * 100)}%`
                      : "–"}
                  </td>
                  {reasons.map((r, j) => {
                    ...
                    return (
                      <td
                        ...
                        data-r={i}
                        data-c={j}
                        data-tip={`${row.label} · ${r}\n${n}명 · 탈락 중 ${(s * 100).toFixed(1)}%`}
                        className="rounded-md px-1.5 py-1.5 text-center"
                        style={{
                          background: n
                            ? `color-mix(in oklab, var(--series-1) ${Math.round(8 + (s / maxShare) * 62)}%, var(--surface))`
                            : undefined,
                        }}
                      >
